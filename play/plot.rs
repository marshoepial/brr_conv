...
pub fn plot(orig: &str, brr: &str, skip: usize) -> Result<()> {
    ...
    let mut brr = BrrIterator::new(BufReader::new(File::open(brr)?).bytes().map(|r| r.expect("Error parsing or smthn"))).skip(skip);
    ...
    for i in 0..=11 {
        for j in -8..=7 {
            ...
            root.draw(&PathElement::new([((x_range.start*x_mod).round() as i32, possible_y), ((x_range.end*x_mod).round() as i32, possible_y)], HSLColor((i as f64)*0.083, 0.5, 0.5)))?;
        }
    }

    // draw zero line
    root.draw(&PathElement::new([((x_range.start*x_mod).round() as i32, y_mid), ((x_range.end*x_mod).round() as i32, y_mid)], &BLACK))?;

    for i in 0..1000 {
        let curr_x = ((i as f32)*x_mod).round() as i32 + x_mid;

        if (i + skip) % 16 == 0 { 
            root.draw(&PathElement::new([(curr_x, 0), (curr_x, y_graph_size as i32)], &BLACK))?;
        }
